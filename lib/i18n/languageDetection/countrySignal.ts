/**
 * Where the visitor's country comes from in a static site.
 *
 * The app is a static export (`output: 'export'`), so no request header ever
 * reaches React — there is no server, middleware or edge function to read
 * `x-vercel-ip-country` or `CF-IPCountry` in. Country therefore has to be
 * handed to the *document*, and this module accepts every cheap way an edge
 * layer can do that:
 *
 ...
 * returns null and detection simply skips the country step — the site works
 * exactly as before. Behind Cloudflare, `requestCountry()` can still recover
 * the code from the same-origin `/cdn-cgi/trace` endpoint, but it is only ever
 * called on the narrow path where the browser offered no supported language
 * at all, so the common visit makes no extra request.
 *
